  ...
  switch (status) {
    case "open":
      ...
      statusBadge.innerHTML =
        '<span class="status-dot online pulse"></span> Open';
...
function updateDeviceStatusUI(data) {
  ...
  if (isOnline) {
    ...
    deviceBadge.innerHTML =
      '<span class="status-dot online pulse"></span> Online';
  } else {
  ...
  lastSeenText.textContent = lastUpdate
    ? "Last seen: " + timeAgo(lastUpdate)
    : "Last seen: —";
}
...
function listenSchedule() {
  ...
  openCloseRef.on("value", (snapshot) => {
    const data = snapshot.val(); // e.g. "0607"

    let openStr = "06:00";
    let closeStr = "19:00";

    if (typeof data === "string" && data.length === 4) {
      openStr = data.substring(0, 2) + ":00";
      closeStr = data.substring(2, 4) + ":00";
    } else if (data && typeof data === "object") {
      openStr = data.open || "06:00";
      closeStr = data.close || "19:00";
    }
...
function listenAccessLogs() {
  ...
  // Do not order by timestamp, just rely on Firebase default chronological push ordering
  accessLogsRef.limitToLast(20).on("child_added", (snapshot) => {
    ...
    // Map ESP fields if missing
    const logData = {
      ...data,
      residentName: data.residentName || data.nama || "Unknown",
      house: data.house || data.alamat || "—",
      status:
        data.status || (data.nama === "Tidak Dikenali" ? "failed" : "success"),
      source: data.source || "rfid",
      uid: data.uid || "—",
    };

    const item = createActivityItem(logData);
    feed.insertBefore(
      item,
      feed.firstChild === emptyState ? emptyState.nextSibling : feed.firstChild,
    );
...
function createActivityItem(data) {
  ...
  const detail =
    data.source === "admin"
      ? "Manual control by Admin"
      : data.source === "auto"
        ? "Automatic schedule"
        : data.status === "failed"
          ? "Access denied — unrecognized RFID"
          : `RFID access — UID: ${data.uid || "—"}`;

  // Use timestamp if exists, else try 'waktu', else '—'
